...
from . import DREM_builder


class DREM(object):
	def __init__(self, data_set, hparams, forward_only=False):
		"""Create the model.
		...
		# Training losses.
		self.loss = DREM_builder.build_graph_and_loss(self)
		...
		if not forward_only:
			...
			# user + query -> product
			self.product_scores, uq_vec = DREM_builder.get_product_scores(self, self.user_idxs, self.query_word_idxs)
			# user + write -> word
			user_vec = tf.nn.embedding_lookup(self.entity_dict['user']['embedding'], self.user_idxs)
			self.uw_scores, uw_vec = DREM_builder.get_relation_scores(self, 0.5, user_vec, 'word', 'word')
			# user + query + write -> word
			self.uqw_scores, uqw_vec = DREM_builder.get_relation_scores(self, 0.5, uq_vec, 'word', 'word')
			# Compute all information based on user + query
			self.uq_entity_list = [
			...
			for key in self.use_relation_dict:
				if self.use_relation_dict[key]:
					...
					scores, vec = DREM_builder.get_relation_scores(self, 0.5, uq_vec, key, tail_entity)
					self.uq_entity_list.append((key, tail_entity, scores))
			'''
			# user + query + also_bought -> product
			self.uqab_scores, uqab_vec = DREM_builder.get_relation_scores(self, 0.5, uq_vec, 'also_bought', 'related_product')
			# user + query + also_viewed -> product
			self.uqav_scores, uqav_vec = DREM_builder.get_relation_scores(self, 0.5, uq_vec, 'also_viewed', 'related_product')
			# user + query + bought_together -> product
			self.uqbt_scores, uqbt_vec = DREM_builder.get_relation_scores(self, 0.5, uq_vec, 'bought_together', 'related_product')
			# user + query + is_brand -> brand
			self.uqib_scores, uqib_vec = DREM_builder.get_relation_scores(self, 0.5, uq_vec, 'brand', 'brand')
			# user + query + is_category -> category
			self.uqic_scores, uqic_vec = DREM_builder.get_relation_scores(self, 0.5, uq_vec, 'categories', 'categories')
			'''
			...
			self.pw_scores, pw_vec = DREM_builder.get_relation_scores(self, 0.5, p_vec, 'word', 'word')
			self.p_entity_list = [
			...
			for key in self.use_relation_dict:
				if self.use_relation_dict[key]:
					tail_entity = self.relation_dict[key]['tail_entity']
					scores, vec = DREM_builder.get_relation_scores(self, 0.5, p_vec, key, tail_entity)
					self.p_entity_list.append((key, tail_entity, scores))
			'''
			# product + also_bought -> product
			self.pab_scores, pab_vec = DREM_builder.get_relation_scores(self, 0.5, p_vec, 'also_bought', 'related_product')
			# product + also_viewed -> product
			self.pav_scores, pav_vec = DREM_builder.get_relation_scores(self, 0.5, p_vec, 'also_viewed', 'related_product')
			# product + bought_together -> product
			self.pbt_scores, pbt_vec = DREM_builder.get_relation_scores(self, 0.5, p_vec, 'bought_together', 'related_product')
			# product + is_brand -> brand
			self.pib_scores, pib_vec = DREM_builder.get_relation_scores(self, 0.5, p_vec, 'brand', 'brand')
	...
	def step(self, session, input_feed, forward_only, file_writer=None, test_mode = 'product_scores'):
		...
		if not forward_only:
			output_feed = [self.updates,	# Update Op that does SGD.
				    self.loss,
                                    self.train_summary]	# Loss for this batch.
		else:
		...
		if not forward_only:
			return outputs[1], outputs[2]	# loss, train_summary
		else:
			if test_mode == 'output_embedding':
